...
bool App::load(){
	...
	// Main render targets
	if ((baseRT   = renderer->addRenderTarget(width,height,FORMAT_RGB10A2)/*(width, height, 1, 1, FORMAT_RGB10A2, 1, SS_NONE)*/) == TEXTURE_NONE) return false;
	if ((normalRT = renderer->addRenderTarget(width, height, FORMAT_RGB10A2)/*(width, height, 1, 1, FORMAT_RGB10A2, 1, SS_NONE)*/) == TEXTURE_NONE) return false;

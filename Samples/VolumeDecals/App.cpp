...
#define ProjectDir	"//Samples/VolumeDecals"
...
bool App::load()
{
	...
	m_Map = new Model();
	if (!m_Map->loadObj(ResDir("/Models/Map.oobj"))) return false;
	...
	if ((m_Ambient     = renderer->addShader(ShaderDir("/Ambient.shd"))) == SHADER_NONE) return false;
	if ((m_Lighting    = renderer->addShader(ShaderDir("/Lighting.shd"))) == SHADER_NONE) return false;
	if ((m_Decal       = renderer->addShader(ShaderDir("/Decals.shd"))) == SHADER_NONE) return false;
	...
	// Textures
	if ((m_BaseTex[0] = renderer->addTexture  (ResDir("/Textures/wood.dds"),                    true, m_BaseFilter)) == TEXTURE_NONE) return false;
	if ((m_BumpTex[0] = renderer->addNormalMap(ResDir("/Textures/woodBump.dds"), FORMAT_RGBA8S, true, m_BaseFilter)) == TEXTURE_NONE) return false;

	if ((m_BaseTex[1] = renderer->addTexture  (ResDir("/Textures/Tx_imp_wall_01_small.dds"),              true, m_BaseFilter)) == TEXTURE_NONE) return false;
	if ((m_BumpTex[1] = renderer->addNormalMap(ResDir("/Textures/Tx_imp_wall_01Bump.dds"), FORMAT_RGBA8S, true, m_BaseFilter)) == TEXTURE_NONE) return false;

	if ((m_BaseTex[2] = renderer->addTexture  (ResDir("/Textures/floor_wood_4.dds"),                    true, m_BaseFilter)) == TEXTURE_NONE) return false;
	if ((m_BumpTex[2] = renderer->addNormalMap(ResDir("/Textures/floor_wood_4Bump.dds"), FORMAT_RGBA8S, true, m_BaseFilter)) == TEXTURE_NONE) return false;

	if ((m_BaseTex[3] = renderer->addTexture  (ResDir("/Textures/floor_wood_3.dds"),                    true, m_BaseFilter)) == TEXTURE_NONE) return false;
	if ((m_BumpTex[3] = renderer->addNormalMap(ResDir("/Textures/floor_wood_3Bump.dds"), FORMAT_RGBA8S, true, m_BaseFilter)) == TEXTURE_NONE) return false;

	if ((m_BaseTex[4] = renderer->addTexture  (ResDir("/Textures/light2.dds"),                    true, m_BaseFilter)) == TEXTURE_NONE) return false;
	if ((m_BumpTex[4] = renderer->addNormalMap(ResDir("/Textures/light2Bump.dds"), FORMAT_RGBA8S, true, m_BaseFilter)) == TEXTURE_NONE) return false;
